...
function ltInterval() {
    ...
    $('.standardPhoto').css('display', 'none');
    $($('.standardPhoto')[bgCount]).css('display', 'block');

    $('.user_pic').css('display', 'none');
    $($('.user_pic')[bgCount]).css('display', 'block');
...
function onDataLoaded(instagram_data) {
    ...
    {
    	$('.standardPhoto')[i].src = photos[i].images.standard_resolution.url;
    	$('.user_pic')[i].src = photos[i].user.profile_picture;
    }
    //console.log(photos);
    if (photos.length > 0) {
        target.empty();
